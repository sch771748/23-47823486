...
# 1. Сначала нужно создать приложение, даже если мы его не запускаем
app = Flask(__name__)
...
<head>
  ...
  <style>
    body { margin: 0; padding: 0; font-family: system-ui; background: #0f0f12; color: #e0e0e0; }
    .section-block { max-width: 1200px; width: 100%; margin: 0 auto; padding: 60px 20px; box-sizing: border-box; opacity: 0; animation: fadeInSection 0.8s ease-out forwards; }

    /* Анимация задержки для каждого блока */
    {% for i in range(blocks|length) %}
    .section-block:nth-child({{ i + 1 }}) { animation-delay: {{ 0.2 * i }}s; }
    {% endfor %}

    p { font-size: 1.1rem; color: rgba(255, 255, 255, 0.95); line-height: 1.5; margin: 0; }
    .hero-title { font-size: clamp(2rem, 5vw, 3rem); line-height: 1.1; margin: 0 0 16px 0; letter-spacing: -1px; }

    /* Макеты */
    .layout-left-text, .layout-right-text { display: flex; align-items: center; justify-content: space-between; gap: 60px; }
    .layout-left-text .hero-text, .layout-right-text .hero-text { flex: 1; min-width: 300px; }
    .layout-left-text .hero-image-wrapper, .layout-right-text .hero-image-wrapper { flex: 1; min-width: 300px; }
    .layout-right-text .hero-text { text-align: right; }

    .layout-top-text-grid-2x2, .layout-center-text-row { display: flex; flex-direction: column; gap: 40px; text-align: center; }
    .layout-center-text-row .hero-images-wrapper { grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); }
    .layout-top-text-grid-2x2 .hero-images-wrapper, .layout-center-text-row .hero-images-wrapper {
      display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; width: 100%; box-sizing: border-box; align-items: start;
    }
    @media (max-width: 768px) {
      .layout-left-text, .layout-right-text { flex-direction: column; align-items: stretch; }
      .layout-right-text .hero-text { text-align: left; }
      .layout-top-text-grid-2x2 .hero-images-wrapper, .layout-center-text-row .hero-images-wrapper { grid-template-columns: 1fr; }
    }

    /* Карточки */
    .hero-image-item, .hero-image-wrapper { position: relative; overflow: hidden; border-radius: 20px; box-shadow: 0 30px 60px rgba(0,0,0,0.5); width: 100%; }
    .hero-image-item img, .hero-image-wrapper img { width: 100%; height: auto; display: block; aspect-ratio: 9 / 16; object-fit: cover; filter: brightness(75%); transition: filter 1s cubic-bezier(0.25, 1, 0.5, 1); }
    .hero-image-item:hover img, .hero-image-wrapper:hover img { filter: brightness(110%); }
    ...
    {% if block.layout == "top-text-grid-2x2" or block.layout == "center-text-row" %}
      ...
      <div class="hero-images-wrapper">
        {% for img_url in block.images %}
          <div class="hero-image-item"><img src="{{ img_url }}" alt="{{ block.title }}"></div>
        {% endfor %}
      </div>
    {% elif block.layout == "left-text" or block.layout == "right-text" %}
      {% if block.layout == "left-text" %}
        <div class="hero-text"><h1 class="hero-title">{{ block.title }}</h1><p>{{ block.text }}</p></div>
        <div class="hero-image-wrapper"><img src="{{ block.images[0] }}" alt="{{ block.title }}"></div>
      {% else %}
        <div class="hero-image-wrapper"><img src="{{ block.images[0] }}" alt="{{ block.title }}"></div>
        <div class="hero-text"><h1 class="hero-title">{{ block.title }}</h1><p>{{ block.text }}</p></div>
      {% endif %}
...
with app.app_context():
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(render_template_string(HTML_TEMPLATE, blocks=blocks))

print("Готово: файл index.html создан в папке проекта!")
